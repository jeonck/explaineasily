...
PAGE = {
    ...
    "panels": [
        ...
        {"svg": P2, "alt": ("골판지 가짜 성문 뒤의 도둑에게 방문객이 6자리 코드를 읽어줌", "A visitor reads a six-digit code aloud to a thief behind a cardboard fake gate"),
         "caption": ("사람이 직접 입력하니까, 피싱 사이트인 가짜 성문에도 입력하게 돼요.", "A person types the code — so they'll type it into a fake gate (a phishing site) too."),
         "small": ('코드 자체는 진짜 성문인지 가짜인지 몰라요. <a href="mfa-ko.html">문지기 이야기</a>의 마지막 장면이에요.',
                   'The code itself can\'t tell a real gate from a fake one. It\'s the last scene of the <a href="mfa-en.html">gatekeeper story</a>.')},
